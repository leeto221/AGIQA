...
__all__ = ['ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet50_multistage' 'resnet101', 'resnet101_multistage'
...
class ResNet(nn.Module):
    ...
    # 在 ResNet 类中，新增：
    def forward_spatial(self, x, return_pooled=True):
        # stem
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        fmap = self.layer4(x)            # <-- 这里是我们要的空间特征, 形如 [B, 2048, H', W'] (通常 7x7)
...
class ResNet_MS(nn.Module):
    ...
    def __init__(self, block, layers, width=1, in_channel=3, zero_init_residual=False,
        ...
        self.total_feats += self.inplanes
        self.ms_layer2 = MultiStageBlock(self.inplanes, norm_layer=None)
        self.layer3 = self._make_layer(block, self.base * 4, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])

        self.total_feats += self.inplanes
        self.ms_layer3 = MultiStageBlock(self.inplanes, norm_layer=None)
        self.layer4 = self._make_layer(block, self.base * 8, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.total_feats += self.inplanes
...
def _resnet_multistage(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNet_MS(block, layers, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        model.load_state_dict(state_dict)
...
if __name__ == '__main__':
    ...
    model = resnet101_multistage(width=1)
